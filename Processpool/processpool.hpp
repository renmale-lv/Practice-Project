...
class processpool{
private:
    //定义私有构造函数，我们只能通过后面的create静态函数类创建processpool实例
    processpool(int listenfd, int process_number = 8);
public:
    //单例模式，程序最多只有一个processpool实例
...
//为信号sig设置信号处理函数
//handler：信号处理函数
//restart：处理结束后是否重新调用被该信号终止的系统调用
...
//统一事件源：将信号通过管道发送给主循环，让主循环执行信号处理函数
template<typename T>
...
//父进程中m_idx值为-1，子进程中m_idx值大于等于0，根据此判断接下来运行的是父进程代码还是子进程代码
template <typename T>
...
void processpool<T>::run_child(){
    ...
    while(!m_stop){
        ...
        for(int i = 0; i < number; ++i){
            ...
            if((sockfd == pipefd) && (events[i].events & EPOLLIN)){
                int client = 0;
                //从父、子进程之间的管道读取数据，并将结果保存在变量client中，如果读取成功，则标识有新客户连接到来
                ...
                if(((ret < 0) && (errno != EAGAIN)) || ret == 0){
                    ...
                    //模板类T必须实现init方法，以初始化一个客户连接，我们直接使用connfd来索引逻辑处理对象（T类型对象）
    ...
    users = NULL;
    close(pipefd);
    //我们将这句话注释掉,以提醒读者:应该由m_listenfd的创建者来关闭这个文件描述符(见后文),即所谓的“对象(比如一个文件描述符,又或者一段堆内存)由哪个函数创建,就应该由哪个函数销毁”
    //close(m_listenfd);
    close(m_epollfd);
}
                        ...
                        case SIGCHLD:{
                            ...
                            while((pid = waitpid(-1, &stat, WNOHANG)) > 0){
                                for(int i = 0; i < m_process_number; ++i){
                                    //如果进程池中第i个子进程退出了，则主进程关闭相应的通信管道，并设置相应的m_pid为-1，以标志子进程已退出
